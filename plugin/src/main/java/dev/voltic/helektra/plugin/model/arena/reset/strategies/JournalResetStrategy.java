...
public class JournalResetStrategy implements ResetStrategyBase {
  ...
  @Inject
  public JournalResetStrategy(IArenaJournalRepository journalRepository,
  ...
  public CompletableFuture<Void> reset(ArenaInstance instance, Arena arena) {
    ...
    List<CompletableFuture<Void>> chunkFutures = new java.util.ArrayList<>();
    ...
    for (int chunkX = minChunkX; chunkX <= maxChunkX; chunkX++) {
      for (int chunkZ = minChunkZ; chunkZ <= maxChunkZ; chunkZ++) {
        ...
        CompletableFuture<Void> chunkFuture = journalRepository.getChanges(instanceId, cx, cz)
            .thenCompose(changes -> {
              if (changes.isEmpty()) {
