...
public class ArenaCommand implements CommandClass {
    ...
    @Inject
    public ArenaCommand(IArenaService arenaService,
    ...
    public void create(@Sender Player player, String id, String name, String typeStr) {
        if (arenaService.getArena(id).isPresent()) {
            player.sendMessage(TranslationUtils.translate("arena.already-exists", "id", id));
            return;
        }
        ArenaType type;
        try {
            ...
            player.sendMessage(TranslationUtils.translate("arena.invalid-type", "type", typeStr));
            return;
        }
        ValidationResult validation = selectionService.validateSelection(player.getUniqueId());
        ...
        Optional<Region> regionOpt = selectionService.getSelectedRegion(player.getUniqueId());
        Optional<Location> spawnAOpt = selectionService.getSpawnA(player.getUniqueId());
        Optional<Location> spawnBOpt = selectionService.getSpawnB(player.getUniqueId());
        if (regionOpt.isEmpty() || spawnAOpt.isEmpty() || spawnBOpt.isEmpty()) {
            player.sendMessage(TranslationUtils.translate("arena.incomplete-selection"));
            return;
        }
        ...
        templateService.createTemplate(arena).thenRun(() -> {
            player.sendMessage(TranslationUtils.translate("arena.created", "id", id, "name", name));
    ...
    public void delete(@Sender CommandSender sender, String id) {
        if (arenaService.getArena(id).isEmpty()) {
            sender.sendMessage(TranslationUtils.translate("arena.not-found", "id", id));
            return;
        }
    ...
    public void list(@Sender CommandSender sender) {
        ...
        sender.sendMessage(TranslationUtils.translate("arena.list-header", "count", arenas.size()));
        arenas.forEach(arena -> {
            int available = poolService.getAvailableCount(arena.getId());
            int inUse = poolService.getInUseCount(arena.getId());
            sender.sendMessage(TranslationUtils.translate("arena.list-format",
                    "id", arena.getId(),
                    "name", arena.getDisplayName(),
                    "type", arena.getType().name(),
    ...
    public void info(@Sender CommandSender sender, String id) {
        ...
        sender.sendMessage(TranslationUtils.translate("arena.info-header", "name", arena.getDisplayName()));
        sender.sendMessage(TranslationUtils.translate("arena.info-id", "id", arena.getId()));
        sender.sendMessage(TranslationUtils.translate("arena.info-type", "type", arena.getType().name()));
        sender.sendMessage(TranslationUtils.translate("arena.info-enabled", "enabled", arena.isEnabled()));
        sender.sendMessage(TranslationUtils.translate("arena.info-pool",
                "available", poolService.getAvailableCount(id),
                "inUse", poolService.getInUseCount(id),
                "resetting", poolService.getResettingCount(id)
        ));
        sender.sendMessage(TranslationUtils.translate("arena.info-metrics",
                "matches", metrics.getTotalMatches(),
                "resets", metrics.getTotalResets(),
                "avgReset", metrics.getAverageResetDurationMs()
        ));
    }
    ...
    public void setSpawn(@Sender Player player, String spawn) {
        ...
        if (spawn.equalsIgnoreCase("a")) {
            ...
            player.sendMessage(TranslationUtils.translate("arena.spawn-set", "spawn", "A"));
        } else if (spawn.equalsIgnoreCase("b")) {
            selectionService.setSpawnB(player.getUniqueId(), location);
            player.sendMessage(TranslationUtils.translate("arena.spawn-set", "spawn", "B"));
        } else {
    ...
    @Command(names = "pool")
    public void pool(@Sender CommandSender sender, String id, String action, Integer value) {
        if (arenaService.getArena(id).isEmpty()) {
            sender.sendMessage(TranslationUtils.translate("arena.not-found", "id", id));
            return;
        }
        switch (action.toLowerCase()) {
            case "scale" -> {
                if (value == null) {
                    sender.sendMessage(TranslationUtils.translate("arena.pool-scale-missing-value"));
                    return;
                }
                poolService.scalePool(id, value);
                sender.sendMessage(TranslationUtils.translate("arena.pool-scaled", "id", id, "size", value));
            }
            case "clear" -> {
                poolService.clearPool(id);
                sender.sendMessage(TranslationUtils.translate("arena.pool-cleared", "id", id));
            }
            default -> sender.sendMessage(TranslationUtils.translate("arena.pool-invalid-action"));
        }
    }
}
