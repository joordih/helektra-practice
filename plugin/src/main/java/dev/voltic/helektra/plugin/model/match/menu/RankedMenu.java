...
public class RankedMenu extends InjectableMenu {
  ...
  @Inject
  public RankedMenu(MenuConfigHelper menuConfig, IKitService kitService, IProfileService profileService) {
  ...
  public void setup(Player player) {
    ...
    profileService.getCachedProfile(player.getUniqueId()).ifPresent(profile -> {
      for (IKit kit : kits) {
        if (slot.get() >= getInventory().getSize())
          break;

        Material material = XMaterial.matchXMaterial(kit.getName().toUpperCase())
            .orElse(XMaterial.DIAMOND_SWORD)
            .get();
        ...
        lore.add("");
        lore.add(TranslationUtils.translate("queue.your-rating", "rating", rating));
  ...
  private void joinRankedQueue(Player player, IKit kit) {
    profileService.getCachedProfile(player.getUniqueId()).ifPresentOrElse(profile -> {
      ...
      if (!profile.getSettings().getPingMatchmaking().enabled() ||
          (player.getPing() >= profile.getSettings().getPingMatchmaking().min() &&
              player.getPing() <= profile.getSettings().getPingMatchmaking().max())) {
        profile.setProfileState(ProfileState.IN_QUEUE);
        kit.incrementQueue();
        player.sendMessage(TranslationUtils.translate("queue.joined-ranked", "kit", kit.getDisplayName()));
        player.closeInventory();
      } else {
