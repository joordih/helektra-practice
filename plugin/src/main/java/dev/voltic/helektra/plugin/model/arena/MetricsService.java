...
public class MetricsService implements IMetricsService {
    ...
    private final Map<String, List<Long>> resetDurations = new ConcurrentHashMap<>();

    @Override
    public ArenaMetrics getMetrics(String arenaId) {
        return metrics.computeIfAbsent(arenaId, id -> ArenaMetrics.builder()
            .arenaId(id)
    ...
    public void recordMatch(String arenaId, long durationMs) {
        ...
        long total = m.getAverageMatchDurationMs() * (m.getTotalMatches() - 1) + durationMs;
        m.setAverageMatchDurationMs(total / m.getTotalMatches());
        m.setLastUpdated(Instant.now());
    }

    @Override
    public void recordReset(String arenaId, ResetStrategy strategy, long durationMs, long blocksModified) {
        ArenaMetrics m = getMetrics(arenaId);
        m.setTotalResets(m.getTotalResets() + 1);
        m.setTotalBlocksModified(m.getTotalBlocksModified() + blocksModified);

        long total = m.getAverageResetDurationMs() * (m.getTotalResets() - 1) + durationMs;
        m.setAverageResetDurationMs(total / m.getTotalResets());

        switch (strategy) {
            case JOURNAL_ONLY -> m.setJournalResets(m.getJournalResets() + 1);
            case SECTION_REWRITE -> m.setSectionResets(m.getSectionResets() + 1);
            case CHUNK_SWAP, FULL_REGENERATE -> m.setChunkSwapResets(m.getChunkSwapResets() + 1);
            case HYBRID -> {}
        }

        resetDurations.computeIfAbsent(arenaId, k -> new ArrayList<>()).add(durationMs);
