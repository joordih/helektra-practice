...
public class MenuConfigHelper {
    ...
    public String getMenuTitle(@NotNull String menuPath) {
        String title = menusConfig.getConfig().getString(menuPath + ".title", "&7Menu");
    ...
    public MenuItemConfig getItemConfig(@NotNull String menuPath, @NotNull String itemKey) {
        String fullPath = menuPath + ".items." + itemKey;
        ConfigurationSection section = menusConfig.getConfig().getConfigurationSection(fullPath);
    ...
    public List<String> getItemKeys(@NotNull String menuPath) {
        ConfigurationSection itemsSection = menusConfig.getConfig().getConfigurationSection(menuPath + ".items");
    ...
    public String getString(@NotNull String path, @NotNull String defaultValue) {
        return ColorUtils.translate(menusConfig.getConfig().getString(path, defaultValue));
    }
    
    public List<String> getStringList(@NotNull String path) {
        return menusConfig.getConfig().getStringList(path).stream()
                .map(ColorUtils::translate)
                .collect(Collectors.toList());
    }
    
    public String replacePlaceholders(@NotNull String text, @NotNull Map<String, String> placeholders) {
    ...
    public List<String> replacePlaceholders(@NotNull List<String> texts, @NotNull Map<String, String> placeholders) {
        return texts.stream()
    ...
    public static class MenuItemConfig {
        ...
        public List<String> getLore() {
            ...
            return section.getStringList("lore").stream()
                    .map(ColorUtils::translate)
                    .collect(Collectors.toList());
        }
        ...
        private Material parseMaterial(String materialName) {
            ...
            if (Helektra.getInstance().getLogger() != null) {
                Helektra.getInstance().getLogger().warning(
                    "Could not find material '" + materialName + "', using STONE as fallback"
                );
            }
